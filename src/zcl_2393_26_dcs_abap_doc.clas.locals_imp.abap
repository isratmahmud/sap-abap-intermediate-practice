...
*    INTO TABLE @connections_buffer.
    ...
    FIELDS carrier_id, connection_id, airport_from_id, airport_to_id, departure_time, arrival_time,
    f~timzone AS timzone_from, t~timzone AS timzone_to,
    div(
    tstmp_seconds_between(
    tstmp1 = dats_tims_to_tstmp( date = @today, time = c~departure_time, tzone = f~timzone ),
    tstmp2 = dats_tims_to_tstmp( date = @today, time = c~arrival_time, tzone = t~timzone ) ), 60 ) AS duration
    INTO TABLE @connections_buffer.
...
*        INTO @connection_details .
      ...
      connection_details = CORRESPONDING #( connections_buffer[ carrier_id = i_carrier_id connection_id = i_connection_id ] ).
...
*    factory method
    ...
    "! @raising zcl_2393_25_exs_failed | Instantiation failed - evaluate the exception text for details.
...
*        e_flight          TYPE REF TO lcl_passenger_flight
    ...
    "! @parameter e_flight | Found flight (object reference)
    "! @parameter e_days_later | Number of days after the requested dateMETHODS find_cargo_flight
...
*      value = CONV #( i_carrier_id ).
        ...
        raise EXCEPTION type zcl_2393_25_exs_failed
        EXPORTING
        textid = zcl_2393_25_exs_failed=>carrier_not_exist
*        previous =
        carrier_id = i_carrier_id.
...
*      textid = cx_abap_auth_check_exception=>missing_authorization.
        ...
        RAISE EXCEPTION TYPE zcl_2393_25_exs_failed
        EXPORTING
        textid = zcl_2393_25_exs_failed=>carrier_no_read_auth
*        previous =
        carrier_id = i_carrier_id.
    ENDIF.
...
*    ) / lines( passenger_flights ).
    ...
    NEXT i += CAST lcl_passenger_flight( <flight> )->get_free_seats( ) ) / pf_count .
